...
solve_day03_part2 <- function(dataDiagnostic){
    ...
    if (is.null(dim(dataTemp))) O2_generator <- sum(dataTemp * 2 ** ((length(dataTemp) - 1):0))
    else O2_generator <- sum(dataTemp[1, ] * 2 ** ((length(dataTemp[1, ]) - 1):0))
    ...
    if (is.null(dim(dataTemp))) CO2_scrubber <- sum(dataTemp * 2 ** ((length(dataTemp) - 1):0))
    else CO2_scrubber <- sum(dataTemp[1, ] * 2 ** ((length(dataTemp[1, ]) - 1):0))
