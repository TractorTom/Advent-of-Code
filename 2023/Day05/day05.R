...
init <- function(data_maps) {
    ...
    while (k <= length(data_maps)) {
        ...
        if (val == "") {
            ...
            name <- data_maps[index_start] |> strsplit(split = " map:") |> unlist()
...
next_category <- function(val, convert_table) {
    for (h in 1:nrow(convert_table)) {
        if (val >= convert_table[h, 2] & val <= convert_table[h, 2] + convert_table[h, 3]) {
...
next_category_intervals <- function(list_intervals, convert_table) {
    ...
    for (h in 1:nrow(convert_table)) {
        ...
        interval_dom <- c(convert_table[h, 2], convert_table[h, 2] + convert_table[h, 3] - 1)
