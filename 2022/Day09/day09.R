...
follow <- function(head, tail) {
    if (any(abs(head - tail) > 1)) {
        ...
        if (x.head == x.tail) return(c(0, (y.head - y.tail) / 2))
        if (y.head == y.tail) return(c((x.head - x.tail) / 2, 0))
        if (x.head > x.tail && y.head > y.tail) return(c(1, 1))
        if (x.head > x.tail && y.head < y.tail) return(c(1, -1))
        if (x.head < x.tail && y.head > y.tail) return(c(-1, 1))
        if (x.head < x.tail && y.head < y.tail) return(c(-1, -1))
    }
