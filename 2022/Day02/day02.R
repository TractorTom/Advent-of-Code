...
translate_table <- data.frame(hand = hands, 
...
solve_day02_part1 <- function(data_game) {
    data_game |> 
        dplyr::rename(elf_choice = 1, 
                      my_choice = 2) |> 
        merge(y = translate_table[, c("hand", "abc_encryption")], 
              by.x = "elf_choice", by.y = "abc_encryption") |> 
        merge(y = translate_table[, c("hand", "xyz_encryption")], 
              by.x = "my_choice", by.y = "xyz_encryption") |> 
        dplyr::rename(my_hand = hand.y, elf_hand = hand.x) |> 
        merge(y = translate_table[, c("hand", "score")], 
...
solve_day02_part2 <- function(data_game) {
    data_game |> 
        dplyr::rename(elf_choice = 1, 
                      enc_result = 2) |> 
        merge(y = translate_table[, c("xyz_encryption", "score_result")], 
              by.x = "enc_result", by.y = "xyz_encryption") |> 
        merge(y = translate_table[, c("abc_encryption", "score_result")], 
              by.x = "elf_choice", by.y = "abc_encryption") |> 
