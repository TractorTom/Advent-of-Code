...
traitement <- function(data_forest) {
    data_forest <- data_forest |> strsplit("") |> do.call(what = rbind)
...
get_scenic_score <- function(matrix_forest, x, y) {
    n <- nrow(matrix_forest)
    if (any(c(x, y) %in% c(1, n))) return(0)
...
get_best_view <- function(matrix_forest) {
    ...
    sapply(1:n, FUN = function(x1) {
        sapply(1:n, FUN = get_scenic_score, 
...
forest_example |> traitement() |> complete_matrix()
forest |>         traitement() |> complete_matrix()

forest_example |> traitement() |> get_best_view()
forest |>         traitement() |> get_best_view()
