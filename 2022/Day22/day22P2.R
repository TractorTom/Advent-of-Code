...
new_direction <- function(direction, instruction) {
    ...
    if (instruction == "R") return(ordre[(which(ordre == direction) %% 4) + 1])
    else if (instruction == "L") return(ordre[((which(ordre == direction) - 2) %% 4) + 1])
...
change_cube <- function(position, nb) {
    ...
    if (cube == 1) {
        if (dire == "left") return(list(nb - y + 1, 1, 4, "right"))
        if (dire == "down") return(list(1, x, 3, "down"))
        if (dire == "up") return(list(x, 1, 6, "right"))
        if (dire == "right") return(list(y, 1, 2, "right"))
    } else if (cube == 2) {
        if (dire == "left") return(list(y, nb, 1, "left"))
        if (dire == "down") return(list(x, nb, 3, "left"))
        if (dire == "up") return(list(nb, x, 6, "up"))
        if (dire == "right") return(list(nb - y + 1, nb, 5, "left"))
    } else if (cube == 3) {
        if (dire == "left") return(list(1, y, 4, "down"))
        if (dire == "down") return(list(1, x, 5, "down"))
        if (dire == "up") return(list(nb, x, 1, "up"))
        if (dire == "right") return(list(nb, y, 2, "up"))
    } else if (cube == 4) {
        if (dire == "left") return(list(nb - y + 1, 1, 1, "right"))
        if (dire == "down") return(list(1, x, 6, "down"))
        if (dire == "up") return(list(x, 1, 3, "right"))
        if (dire == "right") return(list(y, 1, 5, "right"))
    } else if (cube == 5) {
        if (dire == "left") return(list(y, nb, 4, "left"))
        if (dire == "down") return(list(x, nb, 6, "left"))
        if (dire == "up") return(list(nb, x, 3, "up"))
        if (dire == "right") return(list(nb - y + 1, nb, 2, "left"))
    } else if (cube == 6) {
        if (dire == "left") return(list(1, y, 1, "down"))
        if (dire == "down") return(list(1, x, 2, "down"))
        if (dire == "up") return(list(nb, x, 4, "up"))
        if (dire == "right") return(list(nb, y, 5, "up"))
    }
...
while (index < nchar(instr)) {
    ...
    if (index2 == nchar(instr)) val <- substr(instr, index + 1, index2)
    else val <- substr(instr, index + 1, index2 - 1)
    
    list_instr <- rbind(list_instr, c(dire, val))
    index <- index2
}

list_instr <- list_instr |> as.data.frame() |> 
