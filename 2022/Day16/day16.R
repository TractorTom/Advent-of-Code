...
get_neighborhood <- function(data_valves) {
    ...
    for (valve in seq_len(nrow(data_valves))) {
        neighbors <- data_valves[valve, "out"] |> strsplit(",") |> unlist()
...
one_person <- function(flows, distance, pos, score, closed, time) {
    ...
    for (k in closed) {
        ...
        if (score_k > 0) {
            score_tot <- max(score_tot, 
                             one_person(flows, distance, pos = k, score = score + score_k, 
...
solve_day16_part1 <- function(data_valves) {
    ...
    return(one_person(data_valves$flow, distance, pos = starting, score = 0,
...
solve_day16_part2 <- function(data_valves) {
    ...
    return(human_and_elephant(data_valves$flow, distance, start = starting,
