...
compute_rules <- function(data_monkey) {
    ...
    for (monkey_k in seq_len(nb_monkey)) {
        ...
        item <- c(item, list(substr(data_monkey[index + 2], 19, 50) |> 
                                 ...
                                 unlist() |> 
                                 as.numeric()))
        operation <- c(operation, list(function_factory(op = substr(data_monkey[index + 3], 24, 24), 
    ...
    while (round < 10000) {
        ...
        for (k in seq_len(info$nb_monkey)) {
            ...
            if (!is.null(info$item[[k]])) {
                ...
                items[info$item[[k]], ] <- apply(X = items[info$item[[k]], , drop = FALSE],
                                                 MARGIN = 1,
                                                 FUN = \(x) info$op[[k]](x) %% info$test_val) |> t()
